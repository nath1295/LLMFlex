...
class ChatTemplate:
    ...
    def _validate_message_seq(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        Returns:
            List[Dict[str, Any]]: The validated list of messages.
        """
        def format_tool_call(tc, return_dict: bool = False):
            call = tc['function']
            try:
        ...
        if self.allow_multiple_assistant and self.support_tool_call:
            ...
            for msg in messages:
                if msg.get('tool_calls') is not None:
                    msg['tool_calls'] = [format_tool_call(tc, return_dict=True) for tc in msg['tool_calls']]
    ...
    def _apply_chat_template(self, 
        ...
        template = _compile_jinja_template(get_chat_template(self._chat_template, tokenizer=self._tokenizer, tools=tools))
    ...
    def apply_chat_template(self, 
        ...
        if (messages[-1]['role'] == 'assistant') and (messages[-1].get('content')) and continue_final_message:
